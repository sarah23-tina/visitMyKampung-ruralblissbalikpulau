...
if (isset($_GET['edit'])) {
    ...
    $edit_result = mysqli_query($conn, "SELECT * FROM travel_ideas WHERE id = $edit_id AND topic_id = 20");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_idea'])) {
    ...
    if (!empty($_POST['edit_id'])) {
        ...
        $stmt = $conn->prepare("UPDATE travel_ideas SET short_description=?, long_description=?, hemail=? WHERE id=? AND topic_id=20");
        ...
        echo "<script>alert('Food Idea Updated Successfully'); window.location.href='admin_owner_food_ideas.php';</script>";
    } else {
        // Insert new
        $stmt = $conn->prepare("INSERT INTO travel_ideas (topic_id, short_description, long_description, book_now_link, hemail) VALUES (20, ?, ?, '', ?)");
        ...
        echo "<script>alert('Durian Orchard Idea Added Successfully'); window.location.href='admin_durian_owner_travel_ideas.php';</script>";
    }
}
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conn, "DELETE FROM travel_ideas WHERE id = $delete_id AND topic_id=20");
    echo "<script>alert('Durian Orchard Idea Deleted Successfully'); window.location.href='admin_durian_owner_travel_ideas.php';</script>";
}
...
<head>
    <meta charset="UTF-8">
    <title>Manage Duriaan Orchard Ideas</title>
...
<div class="container">
    <h2><?= empty($edit_id) ? 'Add New' : 'Edit' ?> Durian Orchard Idea</h2>
    <form method="post" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-12 text-center">
            <button class="btn btn-primary" name="add_idea"><?= empty($edit_id) ? 'Add Durian Orchard Idea' : 'Update Durian Orchard Idea' ?></button>
        </div>
    </form>

    <h3 class="mt-4">Durian Orchard Ideas List</h3>
        ...
        <?php while ($idea = mysqli_fetch_assoc($ideas_result)): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="?delete=<?= $idea['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this Durian Orchard idea?')">Delete</a>
                </td>
            </tr>
        <?php endwhile; ?>
        </tbody>
