...
if (isset($_GET['edit'])) {
    ...
    $edit_result = mysqli_query($conn, "SELECT * FROM travel_ideas WHERE id = $edit_id AND topic_id = 16");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_idea'])) {
    ...
    if (!empty($_POST['edit_id'])) {
        ...
        $stmt = $conn->prepare("UPDATE travel_ideas SET short_description=?, long_description=?, hemail=? WHERE id=? AND topic_id=16");
        ...
        echo "<script>alert('Food Idea Updated Successfully'); window.location.href='admin_fowner_travel_ideas.php';</script>";
    } else {
        // Insert new
        $stmt = $conn->prepare("INSERT INTO travel_ideas (topic_id, short_description, long_description, book_now_link, hemail) VALUES (16, ?, ?, '', ?)");
        ...
        echo "<script>alert('Food Idea Added Successfully'); window.location.href='admin_fowner_travel_ideas.php';</script>";
    }
}
...
if (isset($_GET['delete'])) {
    ...
    mysqli_query($conn, "DELETE FROM travel_ideas WHERE id = $delete_id AND topic_id=16");
    echo "<script>alert('Food Idea Deleted Successfully'); window.location.href='admin_fowner_travel_ideas.php';</script>";
}
...
<div class="container">
    <h2><?= empty($edit_id) ? 'Add New' : 'Edit' ?> Food Idea</h2>
    ...
    <h3 class="mt-4">Food Ideas List</h3>
        ...
        <?php while ($idea = mysqli_fetch_assoc($ideas_result)): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="?delete=<?= $idea['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this food idea?')">Delete</a>
                </td>
            </tr>
        <?php endwhile; ?>
        </tbody>
